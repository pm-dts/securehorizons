import { mortgageSolutionsData } from "@/../data/mortgage-lending/mortgage-solutions";
import { ArrowRight } from "lucide-react";

export default function MortgageSolutions() {
  return (
    <section id="mortgage-solutions" className="bg-white py-24 scroll-mt-24">
      <div className="mx-auto max-w-[1600px] px-6 md:px-10 xl:px-16">
        ...
        <div className="mx-auto mb-16 max-w-4xl text-center">
          <span className="text-xs font-bold uppercase tracking-[0.22em] text-blue-600">
            {mortgageSolutionsData.badge}
          </span>

          <h2 className="mt-6 text-4xl font-bold tracking-tight text-slate-900 lg:text-5xl">
            {mortgageSolutionsData.title.first}{" "}
            <span className="text-[#0B5FFF]">
              {mortgageSolutionsData.title.highlight}
            </span>
          </h2>

          <p className="mt-6 text-lg leading-8 text-slate-600">
            {mortgageSolutionsData.description}
          </p>
        </div>

        {/* Cards */}
        <div className="grid gap-8 sm:grid-cols-2 xl:grid-cols-3">
          {mortgageSolutionsData.items.map((item) => (
            <div
              key={item.title}
